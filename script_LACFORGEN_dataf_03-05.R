...
system("unzip shp/world_shape_file.zip")
...
dftd <- dftd %>% 
  select('region','Especie','Genero',
         'Nombre_comun','Marcadores')

mapdata <- left_join(world_sf, epp, by="NAME")

mapdata1 <- mapdata %>% filter(!is.na(mapdata$`Estudios por pais`))
...
coromapfin <- leaflet(mapdata1) %>% 
  addTiles() %>% 
  setView(lat = -23.389, lng = -69.433, zoom = 3) %>% 
  addPolygons(
